"""
GVP-Hybrid training: BCE (primary) + communicability auxiliary loss.

loss = BCE(sigmoid(logits), allo_mask, pos_weight=10)
     + lambda_ctqw * communicability_loss(H, active_seed, allo_mask)

At inference on challenge proteins (active site known):
  scores = sigmoid(logits) + beta * normalize(C[active_site → i])

The BCE term provides dense per-node gradient on all 123 ASD proteins.
The communicability term shapes H to route from seed → allosteric sites.
At test time, CTQW boosts residues that are both chemically similar to
allosteric sites AND quantum-reachable from the active site.

Usage:
  python -u train_hybrid.py --processed_dir data/asd_processed --epochs 150
"""
...
from ctqw_torch import ctqw_connectivity_matrix, communicability_loss
...
def bce_loss(scores, allo_mask, pos_weight=10.0):
    ...
    return nn.functional.binary_cross_entropy(scores, labels, weight=weights)


def train_one_epoch(model, samples, optimizer, lambda_ctqw):
    model.train()
    ...
    for sample in samples:
        ...
        optimizer.zero_grad()
        logits, H = model(sample['graph'])
        scores    = torch.sigmoid(logits)

        loss = bce_loss(scores, sample['allosteric_mask'])

        # Auxiliary: communicability loss (centroid seed for ASD proteins)
        if lambda_ctqw > 0:
            active_mask = sample['active_mask']
            if not active_mask.any():
                active_mask = centroid_fallback(sample['coords'], H.device)
            try:
                comm = communicability_loss(H, active_mask, sample['allosteric_mask'])
                if torch.isfinite(comm):
                    loss = loss + lambda_ctqw * comm
            except (torch._C._LinAlgError, Exception):
                pass   # skip ctqw term silently, BCE still trains
    ...
    if n_skip:
        print(f"  [train] {n_skip} skipped")
    return total_loss / max(n_ok, 1)


@torch.no_grad()
def evaluate(model, samples, topk=5, beta=0.5, label=""):
    model.eval()
    results = []
    ...
    for sample in samples:
        ...
        # Boost with CTQW communicability when active site is known
        if active_mask.any():
            try:
                C = ctqw_connectivity_matrix(H)
                active_idx  = active_mask.nonzero(as_tuple=True)[0]
                ctqw_scores = C[active_idx].mean(dim=0).cpu()
                # min-max normalize to [0, 1] before mixing
                lo, hi = ctqw_scores.min(), ctqw_scores.max()
                ctqw_norm = (ctqw_scores - lo) / (hi - lo + 1e-8)
                scores = scores + beta * ctqw_norm
            except (torch._C._LinAlgError, Exception):
                pass   # fall back to BCE scores only
...
def main(args):
    ...
    print(f"Model: {n_params:,} params  λ_ctqw={args.lambda_ctqw}  β={args.beta}")
    ...
    print(f"\nTraining {args.epochs} epochs (BCE + λ={args.lambda_ctqw} comm, β={args.beta} at eval)...")
    for epoch in range(1, args.epochs + 1):
        lr = scheduler.get_last_lr()[0]
        train_loss = train_one_epoch(model, train_samples, optimizer, args.lambda_ctqw)
        scheduler.step()

        if epoch % args.eval_every == 0 or epoch == args.epochs:
            print(f"\n── Epoch {epoch}/{args.epochs}  lr={lr:.2e}  loss={train_loss:.4f} ──")
            _, train_hit = evaluate(model, train_samples[:10], args.topk,
                                    beta=0.0, label="train")   # no CTQW on ASD (no active site)
            _, test_hit  = evaluate(model, test_samples, args.topk,
                                    beta=args.beta, label="test")
            ...
            if test_hit > best_test_hit:
                ...
                ckpt = os.path.join(args.outdir, "best_hybrid.pt")
    ...
    # Also evaluate with beta=0 to see pure BCE contribution
    print("\n── Final eval: β=0 (BCE only) ──")
    evaluate(model, test_samples, args.topk, beta=0.0, label="bce_only")
    print(f"── Final eval: β={args.beta} (BCE + CTQW) ──")
    evaluate(model, test_samples, args.topk, beta=args.beta, label="hybrid")

    print(f"\nDone. Best test hit rate: {best_test_hit:.3f}")
    with open(os.path.join(args.outdir, 'hybrid_log.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    p.add_argument("--lambda_ctqw",    type=float, default=0.1)
    p.add_argument("--beta",           type=float, default=0.5)
    p.add_argument("--device",         default="cuda")
    p.add_argument("--outdir",         default="../outputs/hybrid")
