"""
GVP-GNN + CTQW v2: communicability-based loss.

Replaces time-averaged CTQW prob with closed-form communicability matrix C.
C[i,j] = Σ_k |V_ik|² |V_jk|²  (one eigh call, no time loop — ~100x faster)
Loss = -log( C(active→allo) / (C(active→allo) + C(active→bg)) )

Usage (via SLURM):
  python -u train_asd_v2.py --processed_dir data/asd_processed --epochs 200
"""
...
def centroid_fallback(coords):
    """Return index of residue closest to geometric centroid."""
    coords_cpu = coords.cpu() if isinstance(coords, torch.Tensor) else \
                 torch.tensor(coords, dtype=torch.float32)
    centroid = coords_cpu.mean(dim=0)
    return [(coords_cpu - centroid).norm(dim=-1).argmin().item()]
...
def train_one_epoch(model, samples, optimizer):
    ...
    for sample in samples:
        ...
        if not active_mask.any():
            # Fallback: mark centroid node as active
            idx = centroid_fallback(sample['coords'])[0]
            active_mask = torch.zeros(H.shape[0], dtype=torch.bool, device=H.device)
            active_mask[idx] = True

        try:
            loss = communicability_loss(H, active_mask, sample['allosteric_mask'])
    ...
    if n_skip:
        print(f"  [train] skipped {n_skip} samples this epoch")
...
def evaluate(model, samples, topk=5, label=""):
    ...
    for sample in samples:
        ...
        active_mask_local = active_mask
        if not active_mask_local.any():
            idx = centroid_fallback(coords)[0]
            active_mask_local = torch.zeros(H.shape[0], dtype=torch.bool, device=H.device)
            active_mask_local[idx] = True
        ...
        active_idx = active_mask_local.nonzero(as_tuple=True)[0]
        scores = C[active_idx].mean(dim=0).cpu()   # (N,) mean communicability from active site

        allo_cpu   = allo_mask.cpu()
        active_cpu = active_mask_local.cpu()
        coords_cpu = coords.cpu() if isinstance(coords, torch.Tensor) else \
                     torch.tensor(coords, dtype=torch.float32)

        scores_masked = scores.clone()
        scores_masked[active_cpu] = -1.0
        k = min(topk, scores_masked.shape[0] - 1)
        topk_idx = scores_masked.topk(k).indices.tolist()
...
def main(args):
    ...
    print(f"\nTraining {args.epochs} epochs (communicability loss)...")
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.eval_every == 0 or epoch == args.epochs:
            ...
            if test_hit > best_test_hit:
                ...
                ckpt = os.path.join(args.outdir, "best_model_v2.pt")
    ...
    with open(os.path.join(args.outdir, 'train_v2_log.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    p.add_argument("--outdir",    default="../outputs/train_asd_v2")
