"""
GVP-GNN v2 + CTQW v2: dense Hamiltonian + communicability loss.

H = h @ h^T / K  +  diag(softplus(s))  +  alpha * H_sparse

Dense H ensures:
  - Full rank (no degenerate zero eigenvalues)
  - Rich eigenvalue spectrum → clean eigh gradient
  - Direct gradient path: loss → C → V → h → GVP

Usage (via SLURM):
  python -u train_asd_v3.py --processed_dir data/asd_processed --epochs 200
"""
...
def evaluate(model, samples, topk=5, label=""):
    ...
    for sample in samples:
        ...
        try:
            ...
            print(f"  [skip eval] {sample['name']}: {e}")
            continue

        active_idx = active_mask.nonzero(as_tuple=True)[0]
        scores = C[active_idx].mean(dim=0).cpu()
...
def main(args):
    ...
    print(f"Model: {n_params:,} params  K={args.K}")
    ...
    print(f"\nTraining {args.epochs} epochs (dense H + communicability loss)...")
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.eval_every == 0 or epoch == args.epochs:
            ...
            if test_hit > best_test_hit:
                ...
                ckpt = os.path.join(args.outdir, "best_model_v3.pt")
    ...
    with open(os.path.join(args.outdir, 'train_v3_log.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    p.add_argument("--outdir",    default="../outputs/train_asd_v3")
